...
const Sales = () => {
    ...
    const {queryCategoriesSalees, queryOneCategory, functions, queryMultiDeleteCat} = useContext(FirebaseContext)
    ...
    const listCategory = async() => {
        ...
        await queryCategoriesSalees().onSnapshot((snapshot)=>{
          let allCategory = [];
          let i = 0;
    ...
    const showCategories = () => {
        ...
        const CategoriesList = currentItems.length > 0 ? currentItems.map((item, index) => {
          return(
            ...
            <CardActionArea>
            <CardContent style={{ backgroundColor: "#FFEADF",}}>
    ...
    <Fragment>
        <Container style={{padding:20}} maxWidth="md"> 
            <Typography component="h1" variant="h4" align="center" color="textPrimary" style={{padding:20}}>
                Les Categories salées
            </Typography> 
     
            <div className={classes.heroButtons}>
              <Grid container  justifyContent="center">
                <Grid item>
                  <AddCategories categories={readCategories} typeCat='Salé' title='AJOUTER UNE CATEGORIE SALEE' />
                </Grid>
